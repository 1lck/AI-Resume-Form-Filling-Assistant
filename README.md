...
扩展先维护一份固定 schema 的简历数据，存储在当前设备的 `chrome.storage.local` 中；模型配置仍使用 `chrome.storage.sync`。

简历原文和结构化简历可能超过 Chrome Sync 的单项配额，因此不会再参与跨设备同步。升级到本版本时，扩展会自动把旧版同步存储里的简历数据迁移到本地存储，并清理旧键；模型配置不受影响。
