...
- 日期控件：支持原生 `date/month/datetime-local`，并支持需要“先点开面板再按年/月/日点击”的日期选择器
...
## 日期控件支持说明
当前版本对日期字段做了专门增强，重点覆盖“不能直接写 input.value，必须打开日期面板再点击”的站点：

- 原生输入框：`input[type="date"]`、`input[type="month"]`、`input[type="datetime-local"]`
- Ant Design 旧版日期控件：`ant-calendar-picker`
- Ant Design 新版日期控件：`ant-picker`
- 记忆库回填日期：当你手动填写日期并点击 `保存本页记忆` 后，下次再次填充时，扩展会先把记忆里的值标准化为 `YYYY-MM-DD` / `YYYY-MM`，再走真实日期面板点击链

实现策略不是单纯给日期输入框赋值，而是：

- 先识别日期字段类型和所用组件
- 对需要面板交互的控件，真实触发 `focus / pointerdown / mousedown / mouseup / click`
- 等待可见日期弹层出现
- 按“年 -> 月 -> 日”逐步定位并点击目标日期
- 只有旧版 Ant 单日期控件在点击失败时，才会回退到面板输入框兜底

已验证覆盖：

- 旧版 Ant 日期面板延迟打开
- 旧版 Ant 仅点击 `input.ant-calendar-picker-input` 本身才会打开的场景
- 旧版 Ant 仅真实鼠标事件链才响应的场景
- 新版 Ant `ant-picker` 年/月/日点击路径

如果某个网站日期字段仍无法填写，优先检查：

- 页面是否在 `iframe` 或弹窗中，且弹层尚未真正打开
- 网站是否对日期面板 DOM 做了二次封装，导致标准 Ant 选择器失效
- 该字段是否其实是“日期区间 / 年份选择 / 纯时间选择”，而不是单日期
...
- 日期控件当前优先覆盖单值日期/年月；日期范围、纯年份、纯时间选择器仍未纳入本版支持
- 已对 Ant Design 旧/新日期控件做专门适配，但若站点重写了弹层结构或拦截了事件链，仍可能需要站点级补丁
