...
- 深度扫描（Deep Scan）：自动探索页面上可折叠区块，识别 "添加"、"展开"、"新增" 等关键词，智能匹配简历章节，仅点击简历中有对应数据的展开按钮
...
### 4. 深度扫描只在简历有数据的章节才点击展开按钮

页面上的"添加项目经历"、"添加实习经历"等展开按钮使用关键词匹配。
扫描启动时，扩展会先分析标准简历各章节是否有内容，生成"有数据的章节集合"。
遇到展开按钮时，解析按钮文字（如"添加实习经历"中的"实习"），映射到对应简历章节（`internships`），
如果该章节在简历中没有数据，则跳过不点击。

未匹配到具体章节的通用按钮（如"展开全部"），仍然会被点击。
保守策略：宁可多点，不要漏掉用户可能需要填写的字段。
...
- 多选组
- `contenteditable`
- 动态展开区块（Deep Scan）：自动检测并点击页面上的"添加"、"展开"、"新增"等按钮，展开隐藏字段后再执行扫描填充
- 部分只读日期输入框与日期面板
- 整页/增量/选区三种填充模式
